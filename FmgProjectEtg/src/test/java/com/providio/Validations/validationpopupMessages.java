...
public class validationpopupMessages extends baseClass{
   ...
   public static void validatingProductisAddtoCart(WebDriver driver) {
	   ...
	   if (Stocksize.size() > 0) {
	        ...
	        WebElement addToCart = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[contains(text(),'Successfully added to your cart.')]")));
   ...
   public static void validatingProductisAddtoWishListfromPDP(WebDriver driver) {
	   ...
	   Wait<WebDriver> wait = waitForTheElement.createFluentWait(driver);
       // Wait for the success alert to be visible
       WebElement addTowishList = wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@class='add-to-wishlist-alert text-center alert-success']")));
   ...
   public static void validatingProductisAddtoWishListerror(WebDriver driver) {
       ...
       String actualAddToCart = addTowishList.getText();
       // Expected alert text
       String expectedAddToCart = "The product is already in the wishlist and will not be added again.";
